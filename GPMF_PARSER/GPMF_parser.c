...
GPMF_ERR GPMF_Validate(GPMF_stream *ms, GPMF_LEVELS recurse)
{
	if (ms)
	{
		...
		while (nestsize > 0)
		{
			...
			{
				zprintError("ERROR: uninitized -- GPMF_ERROR_BAD_STRUCTURE");
			...
			if (GPMF_VALID_FOURCC(key))
			{
				...
				if (size + 2 > nestsize)
				{
					zprintError("ERROR: nest size too small within %c%c%c%c-- GPMF_ERROR_BAD_STRUCTURE", PRINTF_4CC(key));
					return GPMF_ERROR_BAD_STRUCTURE;
				}

				if (!GPMF_VALID_FOURCC(key))
				{
					zprintError("ERROR: invalid 4CC -- GPMF_ERROR_BAD_STRUCTURE");
					return GPMF_ERROR_BAD_STRUCTURE;
				}
				...
				if (type == GPMF_TYPE_NEST && recurse == GPMF_RECURSE_LEVELS)
				{
					...
					{
						zprintError("ERROR: nest level within %c%c%c%c too deep -- GPMF_ERROR_BAD_STRUCTURE", PRINTF_4CC(key));
						return GPMF_ERROR_BAD_STRUCTURE;
					}
					...
					{
						zprintError("ERROR: invalid nest within %c%c%c%c -- GPMF_ERROR_BAD_STRUCTURE", PRINTF_4CC(key));
						return GPMF_ERROR_BAD_STRUCTURE;
					}
					else
					{
			...
			else
			{
				...
				{
					zprintError("ERROR: bad struct within %c%c%c%c -- GPMF_ERROR_BAD_STRUCTURE", PRINTF_4CC(key));
					return GPMF_ERROR_BAD_STRUCTURE;
				}
			}
		}
	...
	{
		zprintError("ERROR: Invalid handle -- GPMF_ERROR_MEMORY");
